...
def apple_auth():
    try:
        with open(applefile, 'r') as f:
            cookies = json.load(f)
        headers = apple_is_logged_in(
            cookies['authorization'], cookies['media-user-token'])
        if headers:
...
def apple_is_logged_in(bearer, media):
    ...
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0", "Accept": "*/*", "Accept-Language": "en-US,en;q=0.5", "Accept-Encoding": "gzip, deflate", "Referer": "https://music.apple.com/",
               "Authorization": bearer, "Media-User-Token": media, "Origin": "https://music.apple.com", "Sec-Fetch-Dest": "empty", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Site": "same-site", "Te": "trailers"}
...
def appleapi_get_playlist_content(source_id, headers):
    url = "https://amp-api.music.apple.com:443/v1/me/library/playlists/{}/tracks?l=en-GB".format(
        source_id)
    r = requests.get(url, headers=headers)
    ...
    # If needed, remove duplicates here
    # For example, if each dictionary has an 'id' key that can be used to determine uniqueness:
    seen_ids = set()
    unique_data = []
...
def appleapi_add_playlist_item(dest_id, songid, headers):
    url = "https://amp-api.music.apple.com:443/v1/me/library/playlists/{}/tracks".format(
        dest_id)
